...
- The Town Guard costs 1 food and 1 metal for 2 power, and when a friendly unit in an adjacent lane would be destroyed it is destroyed instead and that unit stands. It gives no power away — it is a shield that is spent, once, and it covers a lost clash, a Fire Sapper and a Ballista bolt alike. A unit leaving of its own accord is not a kill, so nothing is spent on a Sapper burning out or a Mercenary walking away. A Pavise Guard saves itself first, so no Guard is wasted on a unit that would have lived.
- A Town Guard is spent the first time it is needed, and lanes resolve west to east, so with friends falling on both sides only the westmost is saved; flanked by two Guards, only the western one pays. It cannot save itself — a Guard losing its own clash falls like anything else — but two Guards side by side will save each other, since a Guard is a friendly unit like any other. **The save is not free:** spending the Guard empties its lane before that lane resolves, so an enemy standing opposite it walks through to the keep that round.
