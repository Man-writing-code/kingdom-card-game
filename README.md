...
- For testing, choose from fourteen AI opponents and Normal, Hard, or Hardcore decision-making before entering battle. The established Village Uprising, Forest Fire, Strike Steel, Iron Crown, Timber Scholars, and Siege Train remain available alongside eight challenge-ready banners. (The General, Wood Architect and Commons Rush banners are retired for now; their cards remain in the pool.)
...
- Woodland Patrol turns spare cards into Food–Wood tempo, placing Hunting Lodges behind open Huntsmen and using Outriders, Rangers, Feasting Halls, and the Wayfarer’s Lodge to keep pressure moving.
- Granary Muster is the heavier food host: Farms and a pair of Swineherd’s Crofts pay for connected Peasant Mobs, Rabble-Rousers, Levy Captains, and the Granary that strengthens the completed line.
- The Long Spear establishes Pikemen early, then uses Town Guards and Armouries to protect the lanes while the Pikemen grow. Stone Bastion instead plays for endurance with Masons, Gatehouses, a Cathedral, reactive Gaols, and Ballistae.
- Siege Engineers balances Wood and Metal to coordinate Fire Sappers, Battering Rams, Trebuchets, and durable Pavise Guards. Scholars of Ash cycles a Wood-heavy deck through Universities and a Wayfarer’s Lodge until its Fire Sappers find the right targets.
- Tollroad Company is an aggressive Gold banner: Cutpurses and temporary mercenaries force open lanes through Tollhouses. The Gilded Court is the slower capital engine, preserving Treasury and Bank thresholds before converting its reserve into Mercenary Guild Contracts.
...
- Development builds expose the weekly decree pool: The Long Winter, The Lean Court, The Long Muster, and The River Runs High. Choose Calendar Rotation or override it while balancing.
...
- The Gaol costs 4 metal and, when revealed, imprisons the opposing unit in its lane — taken off the board before the clash, so it never fights. It holds exactly one, and once occupied it takes nobody else. The moment it falls, however it falls — pulled down to make room, or broken open by a Ram, a Trebuchet or its own collapsing Ballista — a free copy of the prisoner returns to its owner's hand, costing nothing to deploy and vanishing rather than joining their discard when it next leaves the field. A Town Guard shields against the bailiffs as it does against anything else, and is taken in the prisoner's place.
...
- Village Commons add a temporary Peasant to your hand at the start of each round, whatever the board looks like. Peasant Mobs gain 1 power per friendly unit in an adjacent lane, while the 4-Food Levy Captain immediately musters one generated Peasant into a random empty adjacent unit slot. Food wins by filling connected lanes rather than concentrating everything into one implausible body.
...
- The 60-design global pool is split evenly between 30 buildings and 30 units. It supports Food swarms, Wood defence, pure-Metal elites, Food–Wood hunting and cycling, Metal–Wood siege, Metal–Food entrenchment, and a Gold economy that hires temporary outside force.
- Food–Wood can now spend cards as tempo. A Feasting Hall or Outrider may discard one card during planning for +2 power in the next clash; separate sources stack, but each activates only once and the confirmed discard cannot be undone. A Wayfarer’s Lodge still harvests 1 wood while exchanging a unit for an exact building draw or a building for an exact unit draw. Generated cards vanish when burned instead of entering either pile.
- Militia are especially useful fuel for that planning economy. Discarding one during planning adds a random temporary Farmer or Lumberjack to hand. The volunteer retains its printed deployment cost and vanishes when it leaves play instead of entering the unit pile.
- Gold can be held as capital rather than used only as a wildcard. A Treasury grants an extra selectable draw when a round begins at 3 or more gold. After every normal harvest, each Bank reads the same stored balance and pays 1 gold per complete 2, capped at 6 per Bank. Tollhouses turn direct damage into gold.
- A Mercenary Guild offers a random Mercenary, Assassin, or Lancer Contract each round. Contracts retain their printed Gold costs and vanish when they leave play; a returning Assassin remains a Contract in hand. The 3-gold, 7-power Lancer leaves after its first clash, completing the one-clash Lancer, two-clash Mercenary, and returning Assassin spectrum.
- Assassins and Lancers have **Pierce**. When either wins a unit clash, the difference between its power and the defeated unit's power continues through to the opposing keep. A Town Guard or Pavise Guard may keep the defender alive, but does not stop this overflow; Palisades and fortification protect against it normally.
- The Champion costs 1 food, 1 wood, and 1 metal for 4 power and **Cleave 3**. After every lane has completed its clash, each surviving Champion destroys opposing units with 3 or less current power in adjacent lanes. Those units still fight and block normally that round; Town Guards and an unused Pavise reprieve can protect them.
